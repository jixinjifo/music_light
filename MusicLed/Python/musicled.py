...
def visualize_energy(y):
    global p
    p[:,:] = 100 if y.sum()*20 > 100 else y.sum()*20
    p_filt.update(p)
    p = np.round(p_filt.value)
    # Apply substantial blur to smooth the edges
    p[0, :] = gaussian_filter1d(p[0, :], sigma=4.0)
...
def microphone_update(audio_samples):
    # print("audio_samples:", audio_samples.shape, audio_samples)
    ...
    if vol < config.MIN_VOLUME_THRESHOLD:
        ...
        # print("N, N_zeros:", N, N_zeros)
        # Pad with zeros until the next power of two
        y_data *= fft_window
        # print("y_data:", y_data.shape, y_data)
        y_padded = np.pad(y_data, (0, N_zeros), mode='constant')
        # print("y_padded:", y_padded.shape, y_padded)
        ...
        mel = np.sum(mel, axis=0)
        # print("mel after sum:", mel)
        mel = mel**2.0
        # Gain normalization
        mel_gain.update(np.max(gaussian_filter1d(mel, sigma=1.0)))
        mel /= mel_gain.value
        mel = mel_smoothing.update(mel)
        # print("mel after smoothing:", mel)
